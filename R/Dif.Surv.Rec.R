Dif.Surv.Rec <-
function(XX,type="LRrec",alfa=0,beta=0,gamma=0,eta=0){
...
fit1<-survfitr(Survr(id,time,event)~as.factor(group),data=XL,type="pe")
...
for (z in 1:m) {for (zz in 1:m1){if (X[z,1]== Y[zz,1]){Tiempo1[z,1]<-Y[zz,1];
n.eventG1[z,1]<-NEG1[zz,1];zz<-m1} 
else
{Tiempo1[z,1]<-X[z,1]}}}
AriesgoG1tiempo1<-t(fit1[[Nivelesdefactores[1,1]]]$AtRisk)%*% matrix(1,nrow(fit1[[Nivelesdefactores[1,1]]]$AtRisk),1)
N<-fit1[[Nivelesdefactores[1,1]]]$n+t(fit1[[Nivelesdefactores[1,1]]]$m)%*% matrix(1,nrow(fit1[[Nivelesdefactores[1,1]]]$m),1)
for (z in 1:m) {cuenta[z,1]<-0
for (zz in 1:nrow(tiempocensura) ){if (tiempocensura[zz,1]<=Tiempo1[z,1]) 
cuenta[z,1]<-cuenta[z,1]+1}}
for (z in 1:m-1) {Ncensura[z,1]<--cuenta[z,1]+cuenta[z+1,1]}
Ariesgot0<-N
Ariesgo[1,1]<-N-cuenta[1,1]
AriesgoG1tiempoG1<-t(fit1[[Nivelesdefactores[1,1]]]$AtRisk)%*% matrix(1,nrow(fit1[[Nivelesdefactores[1,1]]]$AtRisk),1)
for (z in 1:m) {for (zz in 1:m1)
{if (X[z,1]== Y[zz,1]) Ariesgo[z,1]<-AriesgoG1tiempoG1[zz,1]}}
for (z in 2:m) {if (Ariesgo[z,1]==0) 
Ariesgo[z,1]<-Ariesgo[z-1,1]-Ncensura[z-1,1]-n.eventG1[z-1,1]}
for (z in 1:m) {AriesgoG1[z,1]<-Ariesgo[z,1]}
AriesgoTotal<-t(fit2$AtRisk)%*% matrix(1,nrow(fit2$AtRisk),1)
NTotal<-fit2$n+t(fit2$m)%*% matrix(1,nrow(fit2$m),1)
G1<-AriesgoG1*(AriesgoTotal-AriesgoG1)*n.event*(AriesgoTotal-n.event)
Varianzan.eventG1<-G1/((AriesgoTotal*AriesgoTotal)*(AriesgoTotal-matrix(1,nrow(n.event),1)))
for (jj in 1:nrow(Varianzan.eventG1)) if (Varianzan.eventG1[jj,1]=="NaN") Varianzan.eventG1[jj,1]<-0
...
GruporeferenciaGG<-cbind(GruporeferenciaGT,AriesgoG1,n.eventG1)
...
Numerador<-t(Pesos*Diferencias)%*%matrix(1,nrow(n.event),1)
Denominador<-t((Pesos*Pesos)*Varianzan.eventG1)%*%matrix(1,nrow(n.event),1)
EstadisticoWilcoxonChi.cuadrado<-Numerador*Numerador/Denominador
p.valorW<-1-pchisq(EstadisticoWilcoxonChi.cuadrado,df=1)
Nomb.Est<-matrix(c("LRrec"))
Estadisticos<-matrix(c(EstadisticoWilcoxonChi.cuadrado))
p.valores<-round(matrix(c(p.valorW)),7)
tabla<-data.frame(Nomb.Est,Chi.cuadrado=Estadisticos,p.valor=p.valores)
print(tabla)},
#____________________________________________________________________
Grec={
PesosGehan<-GruporeferenciaGG[,2]
NumeradorGehan<-t(PesosGehan)%*%Diferencias
DenominadorGehan<-t((PesosGehan*PesosGehan)*Varianzan.eventG1)%*%matrix(1,nrow(n.event),1)
...
DenominadorTaroneWare<-t((PesosTaroneWare*PesosTaroneWare)*Varianzan.eventG1)%*%matrix(1,nrow(n.event),1)
EstadisticoTaroneWare<-NumeradorTaroneWare*NumeradorTaroneWare/DenominadorTaroneWare
p.valorTaroneWare<-1-pchisq(EstadisticoTaroneWare,df=1)
...
DenominadorPetoPeto<-t((PesosPetoPeto*PesosPetoPeto)*Varianzan.eventG1)%*%matrix(1,nrow(n.event),1)
...
supervivencia<-matrix(fit2$survfunc)
nnn<-nrow(supervivencia)
factor<-(GruporeferenciaGG[,2]/(GruporeferenciaGG[,2]+matrix(1,nnn,1)))
Supervivencia<-factor*supervivencia
PesosPetoMod<-Supervivencia
NumeradorPetoMod<-t(PesosPetoMod)%*%Diferencias
DenominadorPetoMod<-t((PesosPetoMod*PesosPetoMod)*Varianzan.eventG1)%*%matrix(1,nrow(n.event),1)
...
Supervivencia<-supervivencia
supe<-Supervivencia
PesosPetoPrentice<-matrix(c(1,supe[1:(nnn-1),1]),nnn,1)
NumeradorPetoPrentice<-t(PesosPetoPrentice)%*%Diferencias
DenominadorPetoPrentice<-t((PesosPetoPrentice*PesosPetoPrentice)*Varianzan.eventG1)%*%matrix(1,nrow(n.event),1)
EstadisticoPetoPrentice<-NumeradorPetoPrentice*NumeradorPetoPrentice/DenominadorPetoPrentice
p.valorPetoPrentice<-1-pchisq(EstadisticoPetoPrentice,df=1)
...
HFrec={
PesosPetoPeto<-Supervivencia
PesosHF<-(PesosPetoPeto)^(alfa)*(matrix(1,nrow(n.event))-PesosPetoPeto)^(beta)
NumeradorHF<-t(PesosHF)%*%Diferencias
DenominadorHF<-t((PesosHF*PesosHF)*Varianzan.eventG1)%*%matrix(1,nrow(n.event),1)
...
PesosPetoPeto<-Supervivencia
PesosGehan<-GruporeferenciaGG[,2]
PesosCM<-(PesosPetoPeto)^(alfa)*(matrix(1,nrow(n.event))-PesosPetoPeto)^(beta)*
(PesosGehan)^(gamma)*(PesosGehan+matrix(1,nrow(n.event),1))^(eta)
NumeradorCM<-t(PesosCM)%*%Diferencias
DenominadorCM<-t((PesosCM*PesosCM)*Varianzan.eventG1)%*%matrix(1,nrow(n.event),1)
...
DenominadorCarlos<-t((PesosCarlos*PesosCarlos)*Varianzan.eventG1)%*%matrix(1,nrow(n.event),1)
...
Estadisticos<-matrix(c(EstadisticoWilcoxonChi.cuadrado,
                       EstadisticoGehan,
                       EstadisticoTaroneWare,
                       EstadisticoPetoPeto,
     EstadisticoPetoMod,
     EstadisticoPetoPrentice,
     EstadisticoHF,
                       EstadisticoCM,
                       EstadisticoCarlos))
p.valores      <-round(matrix(c(p.valorW,
                       p.valorGehan,
                       p.valorTaroneWare,
                       p.valorPetoPeto,
     p.valorPetoMod,
...
tabla<-data.frame(Nomb.Est,Chi.square=Estadisticos,p.value=p.valores)
print(tabla)},
#____________________________________________________________________
